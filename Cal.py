...
class Calcualtor(App):
    def create_button(self,text,pos,method):
        self.num_buttons.append(Button(text=text, pos=pos,font_size=40,size=(120,120)))
        self.parent.add_widget(self.num_buttons[-1])
        self.num_buttons[-1].bind(on_release=method)
    def build(self):
        Window.size = (410, 450)
        self.parent = Widget()
        self.printer = Label(text='Hi', pos=(350, 700), font_size=140)
    ...
    def renew(self, sign=False):
        ...
        if sign:
            ...
            if '.' in text:
                if len(text)<6:
                    try:
                        text=re.findall(self.print_size,text)[0]
                    except:
                        pass
                else:
                    text='...'
            s = text+' '+str(self.print_sign)
        else:
            s = str(self.print)
